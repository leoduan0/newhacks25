...
def get_transactions_by_period(start_date: str, end_date: str, user_id: str = None):
    ...
    try:
        print(
            f"[DEBUG] Fetching transactions from {start_date} to {end_date}, user_id={user_id}"
        )
        ...
        print(
            f"[DEBUG] Querying with datetime range: {start_datetime} to {end_datetime}"
        )

        query = supabase.table("records").select("id, category, purchase_date")

        query = query.gte("purchase_date", start_datetime).lte(
        ...
        print(
            f"[DEBUG] Found {len(response.data) if response.data else 0} transactions"
        )
        ...
        for transaction in transactions:
            try:
                junction_response = (
                    supabase.table("_TransactionItems")
                    .select("A")
                    .eq("B", transaction["id"])
                    .execute()
                )
                ...
                if junction_response.data:
                    ...
                    if item_ids:
                        ...
                        transaction["items"] = (
                            items_response.data if items_response.data else []
                        )
                        print(
                            f"[DEBUG] Transaction {transaction['id'][:8]}... has {len(transaction['items'])} items, total: ${sum(item['cost'] for item in transaction['items']):.2f}"
                        )
                    else:
                        transaction["items"] = []
                else:
                    transaction["items"] = []
                    print(
                        f"[DEBUG] Transaction {transaction['id'][:8]}... has no items in junction table"
                    )
            except Exception as item_error:
                print(
                    f"[ERROR] Error fetching items for transaction {transaction['id']}: {item_error}"
                )
                transaction["items"] = []
...
def calculate_period_stats(start_date: str, end_date: str, user_id: str = None):
    ...
    for transaction in transactions:
        ...
        if transaction.get("items"):
            transaction_total = (
                sum(item["cost"] for item in transaction["items"]) * 1.13
            )

        if category not in stats["categories"]:
            stats["categories"][category] = {"total": 0, "count": 0, "percentage": 0}
...
def get_comparison_stats(
    ...
    if previous_stats["total_spent"] > 0:
        spending_change = (
            (current_stats["total_spent"] - previous_stats["total_spent"])
            / previous_stats["total_spent"]
        ) * 100

    purchases_change = (
        current_stats["total_purchases"] - previous_stats["total_purchases"]
    )
    ...
    if previous_stats["average_purchase"] > 0:
        average_change = (
            (current_stats["average_purchase"] - previous_stats["average_purchase"])
            / previous_stats["average_purchase"]
        ) * 100
